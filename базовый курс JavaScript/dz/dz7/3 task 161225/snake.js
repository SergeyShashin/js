...
const snake = {
  ...
  getNextHeadPoint(borders) {
    let { x, y } = this.body[0];
    switch (this.direction) {
      case 'up':
        y = y - 1 === -1 ? borders.y - 1 : y - 1;
        break;
      case 'down':
        y = y + 1 === borders.y ? 0 : y + 1;
        break;
      case 'left':
        x = x - 1 === -1 ? borders.x - 1 : x - 1;
        break;
      case 'right':
        x = x + 1 === borders.x ? 0 : x + 1;
        break;
    }
    return { x: x, y: y }
  },
...
const count = {
  ...
  init(countHTMLEl, count) {
    this.countHTMLEl = countHTMLEl;
    this.count = count;
    this.render();
  },

  render() {
    this.countHTMLEl.textContent = this.count;
  },
...
const game = {
  ...
  init(userSettings) {
    ...
    this.count.init(document.getElementById('count'), 0);
  ...
  tickInterval() {
    let nextPoint = this.snake.getNextHeadPoint({ x: this.config.getColsCount(), y: this.config.getRowsCount() });

    // if (!this.snakeCanMakeStep(nextPoint) || this.snakeNotHungry()) {
    if (this.snakeNotHungry()) {
      this.finish();
      return
    }
