...
const snake = {
  ...
  makeStep(rowsCount, colsCount) {
    this.body.unshift(this.getNextHeadPoint(rowsCount, colsCount));
    this.body.pop();
  },

  getNextHeadPoint(rowsCount, colsCount) {
    let headPoint = this.body[0];
    switch (this.direction) {
      case 'up':
        if (headPoint.x - 1 === -1) {
          return { x: headPoint.x + rowsCount-1, y: headPoint.y };
        }
        return { x: headPoint.x - 1, y: headPoint.y };
      case 'right':
        if (headPoint.y + 1 === colsCount) {
          return { x: headPoint.x, y: headPoint.y - colsCount+1 };
        }
        return { x: headPoint.x, y: headPoint.y + 1 };
      case 'down':
        if (headPoint.x + 1 === rowsCount) {
          return { x: headPoint.x - rowsCount+1, y: headPoint.y };
        }
        return { x: headPoint.x + 1, y: headPoint.y };
      case 'left':
