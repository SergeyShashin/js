...
const snake = {
  ...
  getNextHeadPoint(borderRight, borderTop) {
    let headPoint = {
      x: this.body[0].x,
      y: this.body[0].y
    };
    switch (this.direction) {
      case 'up':
        headPoint.y = headPoint.y - 1 < 0 ? borderTop - 1 : headPoint.y - 1;
        break;
      case 'down':
        headPoint.y = headPoint.y + 1 === borderTop ? 0 : headPoint.y + 1;
        break;
      case 'right':
        headPoint.x = headPoint.x + 1 === borderRight ? 0 : headPoint.x + 1;
        break;
      case 'left':
        headPoint.x = headPoint.x - 1 < 0 ? borderRight - 1 : headPoint.x - 1;
        break;
    }
    return headPoint;
  },
...
const game = {
  ...
  setTextBtnPlayOrStop(text, isFinish = false) {
    ...
    if (isFinish) {
      ...
      this.playOrStopBtnEl.classList.remove('play');
    } else {
      this.playOrStopBtnEl.classList.add('play');
  ...
  tickInterval() {
    let nextHeadPoint = this.snake.getNextHeadPoint(this.config.getColsCount(), this.config.getRowsCount());
  ...
  canStep(nextHeadPoint) {
    return this.nextStepOnSnake(nextHeadPoint);
  },
