...
const snake = {
   ...
   */
  init(startBody, direction, borders) {
    this.body = startBody;
  ...
  getNextStepHeadPoint() {
    ...
    switch (this.direction) {
      case 'up':
        return { x: firstPoint.x, y: firstPoint.y !== 0 ? firstPoint.y - 1 : this.borders.down};
      case 'right':
        return { x: firstPoint.x !== this.borders.right ? firstPoint.x + 1 : 0, y: firstPoint.y };
      case 'down':
        return { x: firstPoint.x, y: firstPoint.y !== this.borders.down  ? firstPoint.y + 1 : 0 };
      case 'left':
        return { x: firstPoint.x !== 0 ? firstPoint.x - 1 : this.borders.right, y: firstPoint.y };
    }
  },
...
const game = {
  ...
  reset() {
    ...
    this.snake.init(this.getStartSnakeBody(), 'up', { right: this.config.getColsCount()-1, down: this.config.getRowsCount()-1});
