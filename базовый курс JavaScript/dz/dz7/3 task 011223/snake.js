...
const snake = {
  ...
  init(startPosition, direction, maxX, maxY) {
  ...
  getNextHeadPoint() {
    ...
    switch (this.direction) {
      case 'up':
        return { x: x, y: y !== 0 ? --y : this.maxY }
      case 'down':
        return { x: x, y: y !== this.maxY ? ++y : 0 }
      case 'right':
        return { x: x !== this.maxX ? ++x : 0, y: y }
      case 'left':
        return { x: x !== 0 ? --x : this.maxX, y: y }
    }
  },
...
const game = {
  ...
  reset() {
    this.snake.init(this.getStartPositionSnake(), 'up', this.config.getColsCount() - 1, this.config.getRowsCount() - 1);
  ...
  canMakeStep(nextPoint) {
    return !this.nextPoinOnBodySnake(nextPoint);
  },
