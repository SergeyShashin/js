...
const questions = ['Это первый вопрос? (ОК-да/ОТМЕНА-нет)', 'Это воторой вопрос? (ОК-да/ОТМЕНА-нет)', 'Это третий вопрос? (ОК-да/ОТМЕНА-нет)',
  'Это четвёртый вопрос? (ОК-да/ОТМЕНА-нет)', 'Вопросов было 5? (ОК-да/ОТМЕНА-нет)'];
const answers = ['да', 'да', 'да', 'да', 'да'];
...
const game = {
  ...
  run() {
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      let answer = prompt(`${questions[i]}. Для выхода -1.`);

      if (answer === '-1') {
        alert(`Правильных ответов: ${this.count}`);
        alert('До свидания!');
        return
      }
